...
> 装好后版本应为 **4.8**（workflow 会自动把 versionName 改成 4.8；看到 4.8 才说明含原生拍照页）。
...
│   └── PhotoShootActivity.java     # 全屏原生拍照页（连拍、拍完免按「确定」）★v4.9.6 已做内存加固
...
> 判断方法：手机「设置 → 应用信息」看版本号，含原生拍照页的包是 **4.8**。
...
2. 把本目录里的文件**按原结构上传/覆盖**到仓库根目录（**本目录里的内容直接放仓库根，不要再套一层 `App/`**）：
   ...
   - `www/index.html`
   - **`native/*.java`（v4.8 起必须有：原生扫码页 + 原生拍照页）**
   ...
   > git commit -m "v4.8: 原生拍照页 + 原生扫码 + v4.9.6 出图内存加固"
...
2. 装 `app-debug.apk` → 「设置-应用信息」确认：版本 **4.8**、权限列表出现 **相机**。
   - 看到 4.8 = 含原生拍照页：上传页点快门进全屏拍照页，**拍完直接回页面、可连拍、没有「确定」**；
   - 仍是 4.2/4.1 = 旧包（`native/` 没推上去），拍照会走系统相机、多一步「确定」。
...
- 版本号由 workflow 自动改成 4.8（`build.gradle`），装好后看到 4.8 即确认装的是含原生拍照页的新包。
