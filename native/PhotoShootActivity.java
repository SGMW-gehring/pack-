...
public class PhotoShootActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final android.content.SharedPreferences sp = getSharedPreferences("pu_shoot", MODE_PRIVATE);
        crashN = sp.getInt("crash_n", 0);
        try { sp.edit().putInt("crash_n", crashN + 1).apply(); } catch (Throwable ignore) {}
        final boolean safeMode = crashN > 0;
    ...
    private void startCamera() {
        ...
        future.addListener(() -> {
            try {
                ...
                ui.postDelayed(() -> {
                    try { sp.edit().putInt("crash_n", 0).apply(); } catch (Throwable ignore) {}
                }, 2500);
